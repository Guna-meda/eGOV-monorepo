...
def main():
    ...
    consumer = KafkaConsumer(
        ...
        enable_auto_commit=False,  # commit manually, only after a successful DB write
    ...
    for record in consumer:
        try:
            ...
            log.exception("Failed to process message at offset %s -- NOT committing, will retry on next run", record.offset)
            conn.rollback()
            # deliberately don't commit the offset -- next consumer start
            # (same group_id) will reprocess this message rather than silently
            # dropping it.
